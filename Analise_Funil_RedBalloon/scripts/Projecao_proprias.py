# ============================================================================
# PROJEÇÃO DE FUNIL RED BALLOON - UNIDADES PRÓPRIAS (CORRIGIDO)
# Cálculo consistente: soma das partes por unidade
# ============================================================================
# Autor: Carlos Porto de Melo | Marketing Analytics & Growth
# Data: Fev/2026
# Correções: Faixas unificadas (31-90), asterisco para arredondamentos
# ============================================================================
...
KIT_MEDIO = (KIT_KIDS + KIT_JUNIORS_TEENS) / 2  # R$ 1.107,27
...
def classificar_faixa_recencia(dias):
    """Classifica faixa de recência - CORRIGIDO: 31-90 unificado"""
    if pd.isna(dias):
        return None
    elif dias <= 1:
        return 'D-1'
        ...
        return '31-90 dias'  # ✅ CORRIGIDO: Unificado
    else:
        return '> 90 dias'

def formatar_mat_esperadas(valor):
    """Formata matrículas com asterisco se arredondado"""
    if valor == int(valor):
        return f"{int(valor)}"
    else:
        return f"{valor:.1f}*"
...
def calcular_projecao_por_unidade(df, faixa='8-30 dias'):
    ...
    for unidade in UNIDADES_PROPRIAS:
        ...
        resultados.append({
            'Unidade': unidade,
            'Total Leads': n_leads,
            'Matrículas Históricas': int(n_mat),
...
def main():
    ...
    print(f"   Kit médio para não informados: R$ {KIT_MEDIO:,.2f}")
    print("="*70 + "\n")
    ...
    print("="*70)
    print(f"\n{'Faixa':<12} {'Em Qual':>10} {'Mat Esp':>12} {'Receita Média':>15} {'Receita Potencial':>20}")
    print("-"*75)
    for _, row in df_projecao_faixa.iterrows():
        mat_str = formatar_mat_esperadas(row['Matrículas Esperadas'])
        print(f"{row['Faixa Recência']:<12} {row['Em Qualificação']:>10,} {mat_str:>12} R$ {row['Receita Média']:>12,.2f} R$ {row['Receita Potencial']:>17,.2f}")
    
    print("\n" + "="*70)
    print("🏫 PROJEÇÃO POR UNIDADE (Faixa 8-30 dias)")
    print("="*70)
    print(f"\n{'Unidade':<20} {'Em Qual':>10} {'Taxa':>8} {'Mat Esp':>12} {'Receita Potencial':>20}")
    print("-"*75)
    ...
    for _, row in df_projecao_unidade.iterrows():
        mat_str = formatar_mat_esperadas(row['Matrículas Esperadas'])
        print(f"{row['Unidade']:<20} {row['Em Qualificação']:>10,} {row['Taxa Conversão']*100:>7.1f}% {mat_str:>12} R$ {row['Receita Potencial']:>17,.2f}")
    
    print("-"*75)
    mat_total_str = formatar_mat_esperadas(total_mat)
    print(f"{'TOTAL':<20} {total_em_qual:>10,} {'':<8} {mat_total_str:>12} R$ {total_receita:>17,.2f}")
    
    print("\n" + "="*70)
    print("📢 PROJEÇÃO POR CANAL (Faixa 8-30 dias)")
    print("="*70)
    print(f"\n{'Canal':<25} {'Em Qual':>10} {'Taxa':>8} {'Mat Esp':>12} {'Receita Potencial':>20}")
    print("-"*80)
    for _, row in df_projecao_canal.iterrows():
        mat_str = formatar_mat_esperadas(row['Matrículas Esperadas'])
        print(f"{row['Canal']:<25} {row['Em Qualificação']:>10,} {row['Taxa Conversão']*100:>7.1f}% {mat_str:>12} R$ {row['Receita Potencial']:>17,.2f}")
    ...
    print(f"  Matrículas Esperadas:    {mat_total_str}")
    print(f"  Receita Potencial:       R$ {total_receita:,.2f}")
    print("\n  * Valores com asterisco indicam arredondamento")
    print("="*70 + "\n")
